...
module.exports = function createMiddleware({ pool, JWT_SECRET }) {
  ...
  // userId → { v, deleted, expires }. `deleted` is the Migration
  // 053 self-delete tombstone: true means deleted_at IS NOT NULL
  // on the user row, which verifyToken treats as a hard revoke.
  ...
  async function loadEventForEntries(client, eventId) {
    ...
    // Migration 041: post-advance dive-list lock (WA Article
    // 6.7.3). Whichever fires first — entries_close_at OR
