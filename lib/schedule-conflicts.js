...
-- multi-tenant DB doesn't materialise the whole table.
event_divers AS (
...
diver_conflicts AS (
  ...
  -- overlaps) is byte-identical to the previous formulation —
  -- the SELECT projection, GROUP BY, FILTER clauses, and the
  -- LEFT JOIN against users are preserved exactly.
  SELECT
