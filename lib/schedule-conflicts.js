...
diver_conflicts AS (
  ...
  SELECT
    ...
    array_agg(DISTINCT da.user_id) FILTER (WHERE da.user_id IS NOT NULL) AS resource_ids,
    array_agg(DISTINCT COALESCE(u.full_name, u.username)) FILTER (WHERE u.id IS NOT NULL) AS resource_labels,
    p.overlaps
  FROM pairs p
  JOIN LATERAL (
    SELECT dl_a.competitor_id AS user_id
      FROM competitor_dive_lists dl_a
     WHERE dl_a.event_id = p.a_event_id
       AND dl_a.withdrawn_at IS NULL
       AND COALESCE(dl_a.is_reserve, FALSE) = FALSE
       AND dl_a.competitor_id IS NOT NULL
    UNION
    SELECT dl_a.partner_id AS user_id
      FROM competitor_dive_lists dl_a
     WHERE dl_a.event_id = p.a_event_id
       AND dl_a.withdrawn_at IS NULL
       AND COALESCE(dl_a.is_reserve, FALSE) = FALSE
       AND dl_a.partner_id IS NOT NULL
    UNION
    SELECT tm.user_id
      FROM competitor_dive_lists dl_a
      JOIN team_members tm ON tm.team_id = dl_a.team_id
     WHERE dl_a.event_id = p.a_event_id
       AND dl_a.withdrawn_at IS NULL
       AND COALESCE(dl_a.is_reserve, FALSE) = FALSE
       AND dl_a.team_id IS NOT NULL
  ) da ON TRUE
  JOIN LATERAL (
    SELECT dl_b.competitor_id AS user_id
      FROM competitor_dive_lists dl_b
     WHERE dl_b.event_id = p.b_event_id
       AND dl_b.withdrawn_at IS NULL
       AND COALESCE(dl_b.is_reserve, FALSE) = FALSE
       AND dl_b.competitor_id IS NOT NULL
    UNION
    SELECT dl_b.partner_id AS user_id
      FROM competitor_dive_lists dl_b
     WHERE dl_b.event_id = p.b_event_id
       AND dl_b.withdrawn_at IS NULL
       AND COALESCE(dl_b.is_reserve, FALSE) = FALSE
       AND dl_b.partner_id IS NOT NULL
    UNION
    SELECT tm.user_id
      FROM competitor_dive_lists dl_b
      JOIN team_members tm ON tm.team_id = dl_b.team_id
     WHERE dl_b.event_id = p.b_event_id
       AND dl_b.withdrawn_at IS NULL
       AND COALESCE(dl_b.is_reserve, FALSE) = FALSE
       AND dl_b.team_id IS NOT NULL
  ) db ON db.user_id = da.user_id
