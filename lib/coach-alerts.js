...
// competitor id we processed per event; if the new active is the
// same as the last processed (e.g. the operator re-emits state),
// we skip. This means a process restart can re-trigger an alert
// once, which is preferable to never alerting at all if state was
// somehow lost.
//
// Push delivery is fire-and-forget — the helper never throws so
// a push failure can't break the score path. Errors are logged.

const lastProcessedActive = new Map(); // event_id → competitor_id
...
async function maybeNotifyCoachesOfNextDivers(deps, eventId, activePayload) {
  ...
  // Dedupe: same active diver as last fire? Skip.
  if (lastProcessedActive.get(eventId) === activeCompetitorId) return;
  lastProcessedActive.set(eventId, activeCompetitorId);
...
module.exports = { maybeNotifyCoachesOfNextDivers, resetDedupeForTest };
