...
module.exports = function createEmail({ pool }) {
  ...
  async function sendEmailChangeVerify(userId, newEmail, token) {
    ...
    try {
      ...
      await mailer.sendMail({
        from: fromAddress(),
        to: newEmail,
        subject: "Confirm your new Dive Recorder email",
        text: `Hi ${String(fullName)},\n\nYou (or someone using your account) asked to change your Dive Recorder email to this address. Click the link below to confirm — it expires in 30 minutes.\n\n${link}\n\nIf you didn't request this, you can ignore this message; nothing will change.\n\nDive Recorder`,
      });
    } catch (err) {
  ...
  // "Your Dive Recorder email was changed" hygiene email. Fires
  ...
  async function sendEmailChangedNotice(userId, oldEmail, newEmail) {
    ...
    try {
      ...
      await mailer.sendMail({
        from: fromAddress(),
        to: oldEmail,
        subject: "Your Dive Recorder email address was changed",
        text: `Hi ${String(fullName)},\n\nThis is a confirmation that the email address on your Dive Recorder account was just changed to ${masked}. You've also been signed out of every other device for safety.\n\nIf you didn't make this change, contact your organisation admin immediately — your account may be compromised.\n\nDive Recorder`,
      });
    } catch (err) {
