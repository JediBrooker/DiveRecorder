  ...
  async function sendPasswordResetEmail(user, token) {
    ...
    try {
      await mailer.sendMail({
        from: fromAddress(),
        to: user.email,
        subject: "Reset your DivingHQ password",
  ...
  async function sendVerifyEmailEmail(userId, token) {
    ...
    try {
      ...
      await mailer.sendMail({
        from: fromAddress(),
        to: user.email,
        subject: "Verify your DivingHQ email",
  ...
  async function sendEmailChangeVerify(userId, newEmail, token) {
    ...
    try {
      ...
      await mailer.sendMail({
        from: fromAddress(),
        to: newEmail,
        subject: "Confirm your new DivingHQ email",
        text: `Hi ${String(fullName)},\n\nYou (or someone using your account) asked to change your DivingHQ email to this address. Click the link below to confirm — it expires in 30 minutes.\n\n${link}\n\nIf you didn't request this, you can ignore this message; nothing will change.\n\nDivingHQ`,
      });
    } catch (err) {
