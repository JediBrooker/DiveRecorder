...
| Adding any new `$t('…')` key | The same key must exist in `src/locales/en.json` AND every other locale file under `src/locales/`, with a real translation — NOT an English placeholder. **You (Claude in this session) are the translator.** Don't tell the user to run `npm run translate` — they don't always have an API key and that defers the work indefinitely. Do the translations inline, in the same commit that adds the keys. The diving vocabulary is established in each locale (read existing translated strings in the target file to internalise tone). `test/i18n-parity.test.js` enforces this — `npm run test:safe` fails if the english-stuck count exceeds the tolerance budget. |
