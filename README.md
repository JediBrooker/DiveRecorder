...
- **Broadcast / kiosk mode** — three flavours via the Control Room `⋯ → 📺 Broadcast…` chooser: `/control?broadcast=1` (kiosk this screen), `/scoreboard/:eventId/broadcast` (single-event spectator view in a new window), and `/broadcast/all` (auto-grid of every currently-Live event for venues running two pools concurrently — 1 fills the screen, 2 splits 50/50, 3-4 form a 2×2; refreshes its event list every 30 s so newly-Live events join the grid automatically). All variants hide page chrome and scale fonts up to read from the back of a pool deck.
