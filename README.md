...
| `advance.spec.js` | Stage progression API contract (migration 040). Creates a prelim + final pair, hits `GET /api/events/:id/advance/preview` and confirms the child event link + empty `ranked` array (no scores yet). Asserts `POST /api/events/:id/advance` rejects a non-Completed parent (`400 Completed`) and a Completed-but-unscored parent (`400 no scored divers`). Inserts a synthetic reserve row, hits `POST /api/events/:id/reserves/:competitorId/promote` and verifies `is_reserve` flips to false + `display_order` is assigned at the back of the queue + a re-promote on the now-active row returns 404 |
