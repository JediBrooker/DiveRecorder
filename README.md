...
- **Three-stage progression** — Preliminary (all entrants) → Semi-Final (top 18) → Final (top 12). The chain length is operator-defined per event (synchro and team meets typically skip the semi). The "Advance to next stage →" button on a Completed prelim/semi opens an **advance modal** that previews the World Aquatics tie-break ranking, lets the operator pick **top N primaries** + **N reserves** (carry-forward divers who don't compete unless promoted), and chooses a **dive-order mode**: *inherit* (carry the parent's start order forward, drop non-progressors — default for semi-finals), *reverse* (top diver dives last — default for finals), or *random* (re-randomise primaries). Reserves get `is_reserve=true` + `reserve_position`. Idempotent — safe to re-run after a score correction.<br><br>Per **World Aquatics rule DD 7.4 / 7.5**, divers have 30 minutes after the prior stage's results are announced to submit / confirm the next stage's dive list. The advance endpoint stamps `events.dive_list_locks_at = NOW() + lock_minutes` (default 30, configurable per advance) and pushes a `dive_list_advanced` notification to each advanced diver. Past the deadline the diver portal locks; their inherited list rides as the default. The diver portal surfaces a **Confirm or edit by [time]** banner — clicking *Confirm inherited list* hits `/api/competitor/confirm-list` and stamps `confirmed_at` on every row so the operator can audit who actively responded. Editing any round and re-submitting also stamps `confirmed_at` (the submit-list endpoint upserts now). The meet manager retains a late-entry bypass via `POST /api/events/:id/roster` (Control Room) for changes after the lock.<br><br>Per **World Aquatics rule DD 9.1 / 9.2**, when a primary withdraws before the event begins the next-highest-ranked reserve takes their place — and crucially **inherits the withdrawn diver's start position** so the dive sequence is preserved. The Control Room renders a **Reserves panel** (between Up Next and Standings) listing each reserve with their reserve_position, club, and a per-row picker: *Slot at back of queue* (default), *Replace withdrawn [name]*, or *Replace active [name] (will withdraw them)*. Clicking *Promote* hits `POST /api/events/:id/reserves/:competitorId/promote` with the optional `replaces_competitor_id` — the server stamps `withdrawn_at` on the replaced primary, clears their `display_order`, and assigns it to the reserve. Diver-side, `CompetitorView` surfaces an amber **"You're Reserve N"** banner so reserves know to keep their list current; `/api/competitor/list-status?event_id=…` powers it. Audit row `reserve.replaced_diver` records every swap with the replaced name + new display_order.
