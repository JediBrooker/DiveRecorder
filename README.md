...
- **Broadcast / kiosk mode** — three flavours via the Control Room `⋯ → 📺 Broadcast…` chooser: `/control?broadcast=1` (kiosk this screen), `/scoreboard/:eventId/broadcast` (single-event spectator view in a new window), and `/broadcast/all?ids=…` (auto-grid of a chosen subset of currently-Live events for venues running two-plus pools concurrently — 1 fills the screen, 2 splits 50/50, 3-4 form a 2×2; refreshes its event list every 30 s). The third option expands an in-modal sub-picker so the operator ticks which events to project (every Live event ticked by default; the URL drops the `?ids=` filter when *all* are selected so newly-Live events still auto-join). When every event in a picked subset finishes the page swaps to a one-click "Show all Live events" rescue. All variants hide page chrome and scale fonts up to read from the back of a pool deck.
