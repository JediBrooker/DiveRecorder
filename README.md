...
- **Three-stage progression** — Preliminary (all entrants) → Semi-Final (top 18) → Final (top 12). The chain length is operator-defined per event (synchro and team meets typically skip the semi). The "Advance to next stage →" button on a Completed prelim/semi opens an **advance modal** that previews the World Aquatics tie-break ranking, lets the operator pick **top N primaries** + **N reserves** (carry-forward divers who don't compete unless promoted), and chooses a **dive-order mode**: *inherit* (carry the parent's start order forward, drop non-progressors — default for semi-finals), *reverse* (top diver dives last — default for finals per **World Aquatics Rule 2.1.3** reverse-rank start order), or *random* (re-randomise primaries). Reserves get `is_reserve=true` + `reserve_position`. Idempotent — safe to re-run after a score correction.<br><br>Per the **dive-list submission window** of **Rule 2.1.3**, divers can change their list up to 30 minutes before the next stage begins. The advance endpoint stamps `events.dive_list_locks_at` to `child.scheduled_at − lock_minutes` (default 30) when the child has a `scheduled_at`, falling back to `NOW() + lock_minutes` when it doesn't. Past the deadline the diver portal locks; their inherited list rides as the default. The diver portal surfaces a **Confirm or edit by [time]** banner — clicking *Confirm inherited list* hits `/api/competitor/confirm-list` and stamps `confirmed_at` on every row so the operator can audit who actively responded. Editing any round and re-submitting also stamps `confirmed_at` (the submit-list endpoint upserts now). The meet manager retains a late-entry bypass via `POST /api/events/:id/roster` (Control Room) for changes after the lock.<br><br>Per **Rule 2.1.6 (advancement)** + **Rule 2.1.3 (reverse-rank start order)**, when a primary withdraws from a final the next-highest-ranked reserve takes their place — and because the reserve has the **worst qualifying rank** in the new field, they **dive first** (`display_order = 1`). Every primary qualified worse than the replaced diver shifts +1 to make room; the highest-ranked diver still dives last. For semi-finals the inherit-slot fallback applies (semis don't follow reverse-rank). The Control Room renders a **Reserves panel** (between Up Next and Standings) listing each reserve with their reserve_position, club, and a per-row picker: *Slot at back of queue* (default), *Replace withdrawn [name]*, or *Replace active [name] (will withdraw them)*. Clicking *Promote* hits `POST /api/events/:id/reserves/:competitorId/promote` with the optional `replaces_competitor_id` — the server applies the right algorithm based on `event_format`. Diver-side, `CompetitorView` surfaces an amber **"You're Reserve N"** banner so reserves know to keep their list current; `/api/competitor/list-status?event_id=…` powers it. Audit row `reserve.replaced_diver` records every swap.
