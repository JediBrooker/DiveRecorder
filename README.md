...
When a meet is over, the Scoreboard switches to a recap layout: podium spotlight, full standings with club + team lines, and a per-diver dive-by-dive breakdown. Per-judge scores are colour-coded by World Aquatics category (excellent → failed) with the trim rule visualised by struck-through dimmed scores. A **Judge Ranking Analysis** card sits between the standings and the highlights panel: expand it to see what each diver's rank would have been if every judge on the panel had scored unanimously like one specific judge — the cells where a judge's hypothetical rank differs from the official one by two or more positions are highlighted in cyan so the columns that would have re-shuffled the podium jump out, and CSV / PDF exports are one click away. Hovering a score chip elsewhere on the page also now spells out "Ranked this dive Nth of M in round R" alongside the existing judge identity line. **PDF / CSV / Start List** buttons in the header export the recap in print-ready form.
...
- **Pre-flight review modal** — clicking the green Start Event button opens a last-chance summary listing roster size, judge panel composition, referee status, and any warnings (synchro on a 5-judge panel, divers with incomplete dive lists, partial panel, missing referee). Catches misconfigurations before the event broadcasts to spectator scoreboards.
- **State-aware Finalise button** — the header Finalise control hides during pre-meet (`Start Event` is the only meaningful click), tucks into the `⋯` menu as `✓ Finalise event early…` during a Live meet, and surfaces prominently only at the natural completion moment. Mirrors the centre-column Next Diver button which also morphs into `✓ Event Complete — Finalise & View Results` once the last dive is scored.
- **60-second shot clock** auto-starts when a diver is set; pause/reset; visual amber/red countdown; audible alert at 0.
...
- **Score correction with live preview** — click any completed dive in the Control Room history; modal lets the manager pick a judge, edit the value, and provide a reason. The **live preview** recomputes the trim sum + dive points (with the WA × 0.6 synchro factor) on every keystroke so the operator sees the impact before clicking Save. A small note flags when the edit shifts the trim selection (a different judge gets dropped). Audit-logged with old/new values, actor, IP, user agent.
...
- **Three-stage progression** — Preliminary (all entrants) → Semi-Final (top 18) → Final (top 12). The chain length is operator-defined per event (synchro and team meets typically skip the semi). The "Advance to next stage →" button on a Completed prelim/semi opens an **advance modal** that previews the World Aquatics tie-break ranking, lets the operator pick **top N primaries** + **N reserves** (carry-forward divers who don't compete unless promoted), and chooses a **dive-order mode**: *reverse* (top diver dives last — **WA default per Articles 4.1.8 + 4.1.10** for both semi-finals and finals), *inherit* (carry the parent's start order forward, drop non-progressors — escape hatch for non-WA-sanctioned events), or *random* (re-randomise primaries). Reserves get `is_reserve=true` + `reserve_position`. Idempotent — safe to re-run after a score correction. **Re-seeding refuses with a 409 when the target event already has recorded scores** (advance + every Super Final seed endpoint), so an operator who flipped a Live event back to Upcoming and ran the advance again can't silently destroy scored rounds via the `competitor_dive_lists → scores` FK cascade.<br><br>Per **Article 6.7.3** divers may change their dive list "no later than thirty (30) minutes after the end of the previous stage of the event". The advance endpoint stamps `events.dive_list_locks_at = NOW() + lock_minutes` (default 30, configurable per advance, 0 = no auto-lock); since advance runs after the parent is Completed, `NOW()` ≈ "end of previous stage". Past the deadline the diver portal locks; the inherited list rides as the default. The diver portal surfaces a **Confirm or edit by [time]** banner — clicking *Confirm inherited list* hits `/api/competitor/confirm-list` and stamps `confirmed_at` on every row so the operator can audit who actively responded. Editing any round and re-submitting also stamps `confirmed_at` (the submit-list endpoint upserts now). The meet manager retains a late-entry bypass via `POST /api/events/:id/roster` (Control Room) for changes after the lock.<br><br>Per **Article 4.1.12 (advancement)** combined with **Articles 4.1.8 + 4.1.10 (reverse-rank start order)**, when a primary withdraws the next-highest-ranked reserve takes their place — and because the reserve has the **worst qualifying rank** in the new field, they **dive first** (`display_order = 1`). Every primary qualified worse than the replaced diver shifts +1 to make room; the highest-ranked diver still dives last. The same algorithm applies to both semi-finals and finals (WA reverse-rank governs all subsequent stages). The Control Room renders a **Reserves panel** (between Up Next and Standings) listing each reserve with their reserve_position, club, and a per-row picker: *Slot at back of queue* (default), *Replace withdrawn [name]*, or *Replace active [name] (will withdraw them)*. Clicking *Promote* hits `POST /api/events/:id/reserves/:competitorId/promote` with the optional `replaces_competitor_id`. Diver-side, `CompetitorView` surfaces an amber **"You're Reserve N"** banner so reserves know to keep their list current; `/api/competitor/list-status?event_id=…` powers it. Audit row `reserve.replaced_diver` records every swap.
- **Diving World Cup Super Final** (`docs/2026.03.05-…-Super-Final…pdf` Appendix 3) — three-stage knockout replacing the regular Final for the season-ending World Cup. Top 12 from Stop 1 (max 2 per Federation) seed into 6 head-to-head pairs (12v1, 11v2, 10v3, 9v4, 8v5, 7v6) split into two physical groups (G1 = pairs 12v1, 9v4, 8v5; G2 = pairs 11v2, 10v3, 7v6). Each diver does 3 dives; higher pair total advances → 6 winners regroup into a Semi Final (cumulative score with H2H carries forward, women add 2 dives, men add 3, top 2 per group advance). Final has 4 divers diving their full list (5W / 6M) with **scores reset to zero**. Final 1–12 ranking blends three sources: positions 1–4 from Final scores, 5–6 from H2H+SF cumulative, 7–12 from H2H scores only. **Dive-offs** (Appendix 3 §6) — single-dive tie-breakers held at end of H2H or SF on a tie; Control Room operator picks the dive each tied diver redoes, records the score, declares the winner; doesn't affect official scores. **Synchro reserve pool** (§5.1) — if a Top-12 individual withdraws after Team Leaders Meeting, the Control Room can pull a replacement from the same meet's synchronised events (federation-priority listing, won't add a 3rd diver to any country). **Modified change-of-dives window** (§4.1) — F stage uses a 15-min lock with a 5-min buffer before start (vs. the regular 30-min Article 6.7.3 default); the diver portal shows an aggressive amber MM:SS countdown banner so the diver doesn't miss the shorter deadline.
- **Age groups / divisions** — structured dropdown that shows the WA Group letter alongside its age band: *Group D — 11 and under*, *Group C — 12/13*, *Group B — 14/15*, *Group A — 16-18* (anchored to **PART FOUR Article 13** — A=13.2.2, B=13.2.1, C=13.3.1; D extends WA scheme down per common national-federation usage), plus Masters / Open / Other (custom). Stored as the canonical short form (`Junior Group A`, `Open`, `Masters 30-34`, etc.) so existing rows + templates keep matching.
...
### Judge analysis (transparency)

Every judge has a **public** analytics page at `/judge-profile/:id` and a discoverable directory at `/judges`. Same transparency stance as the diver profile — every score the rollups aggregate is already visible on the live scoreboard, archived meet pages, and PDF score sheets; pre-aggregating per-judge just makes patterns visible (per-country bias, per-club drift) instead of leaving them buried in 300 rows of per-dive HTML.

The numeric reference for every metric is the **panel-kept mean** — the arithmetic mean of the post-trim scores for each dive (PART FOUR Article 13 trim rules). That's the same kept set the dive-points formula uses, so a judge's deviation from it is the same signal a WA assessor would compute by hand.

Default widget catalog (drag to reorder, persisted per-user for the judge themselves):

- **Bias Summary** — headline mean signed deviation, MAD, stddev, sample size.
- **Deviation Distribution** — histogram of (judge_score − kept_mean) bucketed at 0.5-point bands.
- **Agreement Rate** — % of calls within ±0.5 (one increment per Article 7.9.4) and ±1.0.
- **Drop Rate** — % trimmed off the panel + the high-vs-low split (a hi-bias pattern is exactly the signal Article 8.4.9 cares about).
- **By Board Height** / **By Dive Group** / **By DD Difficulty** / **By Round**.
- **By Diver Country** / **By Diver Club** / **By Individual Diver** — top-N by absolute deviation (cross-country bias signal — relevant to Article 7.4 which already restricts the FINAL panel to non-same-nationality judges).
- **Recent Meets** — last 10 events officiated with deviation, dive count, drop rate.
- **Trend Over Time** — weekly mean signed deviation.
- **Panel Comparison** — your average score vs the panel-kept mean across all comparable dives.

**Score chips link straight through.** Every per-judge score on the scoreboard (live or completed) is a clickable chip. Hover shows the judge's identity (`J3 — Maria Schmidt · GER · Munich Diving Club`) plus a click hint; click navigates to that judge's analysis page. Tooltips render instantly via a CSS-only `[data-tip]::after` pseudo-element (the global `v-tip` directive at `src/directives/tip.js` swaps every native `title=` for the same fast bubble — used everywhere across the app, not just the scoreboard).
...
- **International events** — a host org can opt other federations onto an event's participating list (Meet Manager → ⋯ → Federations…). Foreign divers self-enter without a shadow account; their results count toward **their home federation's** records (`lib/records.js` already keys federation records off `users.org_id`). The host keeps full control of meet_manager / referee / score correction / audit log. Empty list = domestic-only event. See migration 036.
  - **Foreign judges**. The judge picker for an international meet pulls from every participating federation, not just the host (`/api/events/:id/eligible-judges`); the assignment endpoint validates each judge's home org matches the host or one of the participating orgs. Every judge tile shows a country chip so seating is unambiguous.
  - **Invite notifications**. Adding a federation fires an in-app + push notification (`category: "international_invite"`) to every org_admin in the invited org. Removing one — including a visiting federation self-withdrawing — notifies the host's admins.
  - **Visiting federation withdraws**. A guest org's admin can withdraw their own federation from the host's overflow menu (Manager → ⋯ → Withdraw participation). Existing dive list rows from their divers stay intact — only NEW entries are blocked.
  - **Synchro across federations**. The synchro-partner check accepts a partner from any federation eligible to enter the event (host or participating). Defaults to "same org as the entrant" for domestic meets — no behaviour change there.
  - **Pulse strip 🌐 INVITED chip**. Visiting org admins / divers see a cyan pulse chip on their dashboard when their federation is invited to one or more foreign-hosted events. Click → jumps to the event.
  - **Public meet page**. Renders a "🌐 Hosted by X · Visiting: AUS NZL FIJ" strip when one or more federations are invited, so spectators landing on `/meet/:id` see immediately who's competing.
  - **Country medal table**. The recap card grid gains an Olympic-style medal table when ≥2 distinct countries appear in the standings. Sorted by gold count, then silver, then bronze.
  - **Continental records** (migration 037). New `records_continental` + `records_continental_history` tables alongside personal/club/federation. Each federation gets a `continent` column (`africa | americas | asia | europe | oceania`); divers from a classified federation have their dives compared against the continental record book in addition to the federation one. A junior setting an Oceania record at a Pacific Junior Champs now has a place to land.
...
- **Role-aware tabbed dashboard** — pulse strip across the top gives a cross-role glance regardless of active tab. Each chip carries a glyph + count (`🔴 3 LIVE`, `📅 2 UPCOMING`, `👥 5 PENDING`, …), is clickable to jump to that role's tab, and shows a hover popover listing the actual items — each item is a deep-link to the specific event / user / panel. Items get urgency markers (amber "closing soon" for events with <24h entries window; red "overdue" for role requests older than 7 days). The LIVE chip breathes while there are live events; counts that change flash cyan. A latest-activity ticker on the right auto-cycles through recent audit rows. The strip is socket-driven — event-status flips and new role requests update the counts immediately rather than waiting for the next 30 s poll. Below the strip, one tab per role the user holds + an Other tab; each tab badge-counts pending work. Smart-pick auto-selects the initial tab using a hierarchy of signals (LIVE event → operator tab; imminent entries close → diver tab; pending governance work → org admin tab; localStorage stamp; most-privileged role fallback). Diver search, My Profile, and Sign Out live in the top-right account row.
...
- **Search + status filter on the Meet Manager event list** — once an org has run a season, free-text search and `Upcoming / Live / Completed` filter chips make a 30+ row list instantly scannable. Per-status counts on the chips; auto-hidden under 4 events; "Clear filters" rescue link if every filter rules everything out.
- **First-run setup wizard** at `/setup` — new federations land on an empty dashboard with no obvious starting point; the wizard walks them through `Welcome → Create your first club → Invite your people → Build your first event` with a copy-able registration link in step 3. Each step is skip-able; localStorage stamps so a returning admin isn't redirected on every visit.
