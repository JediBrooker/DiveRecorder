...
- **Role-aware tabbed dashboard** — pulse strip across the top (`3 LIVE · 2 UPCOMING · 5 PENDING · entries close 14d`) gives a cross-role glance regardless of active tab. Below it, one tab per role the user holds + an Other tab; each tab badge-counts the pending work in that role. Active tab renders role-scoped content: org admins see attention cards (live + upcoming events + pending role requests / org registrations) plus a recent-activity feed; meet managers see their events list; divers see "Your next meet"; etc. Smart-pick auto-selects the initial tab using a hierarchy of signals (LIVE event → operator tab; imminent entries close → diver tab; pending governance work → org admin tab; localStorage stamp; most-privileged role fallback). Diver search, My Profile, and Sign Out live in the top-right account row.
